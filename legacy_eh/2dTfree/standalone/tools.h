...
double Tsint(double **T,int nx,int ny) {
...
double Ts(double **T,int nx,int ny) {
...
void draw_viguetabovedilla2rellena(int nx,int ny,int **NT,int i1,int j1,int i2,int j2) {
  ...
  for (j = j1; j < j2 ; ++j)
   ...
   for (j = j1; j < j2 ; ++j)
    for ( i = i1; i < i2 ; ++i)
      NT[i][j] = 14;  //para establecer que es relleno y luego debe ser borrada
...
void abrefile_indice(char *user,int cont_herramienta,int month,
		     FILE **f_in,
		     double Tsamax,double Tsamin,double Tintmax,double Tintmin,double t_Tintmax,
		     double t_Tsamax,double Qin,double Tintaverage,double dt,double X,
...
void abrefile(char *user,int month,int cont_herramienta,FILE **f_in) {
  ...
  *f_in = fopen(s_file_capas,"w");
  fprintf(*f_in,"t\tIs\tTsa\tTa\tT_paredext\tTparedint\tTint\tT_n\tDeltaT_n\n");
  fprintf(*f_in,"[h]\t[W/m2]\t[oC]\t[oC]\t[oC]\t[oC]\t[oC]\t[oC]\t[oC]\n");
...
void max_min(double *Tsamax,double *Tsamin,double *Tintmax,double *Tintmin,
	double *t_Tintmax, double *t_Tsamax,double t,double Tsa,double Tint,double *Tsi,double **T,int nx,int ny) {
...
int eh_inner_iters = 0;   /* nº de iteraciones del lazo interno (instrumentación Fase 3) */
...
void solve_PQ (double **a,double **b,double **c,double **d,double *P,double *Q,double **Tn,double **T,int nx,int ny,
	  ...
	  double dx,double dy,double **k,double **rhoc,int **NT,double Tsa,double ho,double **To,double X,double t, double *Tintaverage,
	  double *Thueco,double *hh,
	  int i1, int j1, int i2, int j2,double a21,double e22,
	       double E,double *Qrup,double *Qrdown, double tipo,double beta,
	       double *Tarriba,double *Tabajo,double *Nur) {
  int i,j;
  ...
  for (i = 0; i < nx; ++i) {
    Qh += hi*dt*dx*(T[i][ny-1]-Ti); 
    //printf("Qh = %f\n",Qh);
    if (T[i][ny-1] > *Tint)
      *Qin += hi*dx*(T[i][ny-1]-Ti);
  }
...
/* PostgreSQL session bookkeeping removed for standalone build.
   These stubs keep the original call sites in conduction.c intact. */
database_begin(char *user,int cont_herramienta,int month, char *baseDatos) {
  (void)user; (void)cont_herramienta; (void)month; (void)baseDatos;
}
