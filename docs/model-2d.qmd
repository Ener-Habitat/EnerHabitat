...
thickness. By periodicity, no heat flows between neighbouring cells: the lateral sides of
the cell are **adiabatic** (symmetry planes).
...
the inside ($y = e$), as in the cross-section figures below. Note that the reference
...
average day, same `Tsa()`. The indoor air $T_i$ follows the same two solution modes:
free-running (lumped air node, now fed by the width-averaged flux from the indoor
surface) or air-conditioned ($T_i = T_n$ constant).

## Cavity physics (`Fill.AIR`)

When a cell is an air cavity, three heat-transfer mechanisms act together
[@barrios2016]: conduction through the surrounding solids, **natural convection** of the
cavity air, and **radiation** between the cavity surfaces.
...
surfaces are grey and diffuse with emissivity $\varepsilon$ (the `emissivity` parameter,
0.9 by default), and each surface emits at its mean temperature [@incropera2007]. The
net exchange between surfaces $m$ and $n$ is

$$
q_{m \to n} \;=\; \varepsilon\, \sigma\, F_{mn} \left( T_m^4 - T_n^4 \right),
$$ {#eq-rad}

with $\sigma$ the Stefan–Boltzmann constant and $F_{mn}$ the view factor between them.
...
- Orientation is validated: `HollowBlock` requires `tilt = 90`, `Slab` requires
  `tilt = 0`.
- Outputs are those of the 1D model — `Ti`, `energy_transfer`, `cooling_energy`,
  `heating_energy` — expressed **per unit of indoor surface area** (the width-averaged
  flux at $y = e$), so 1D and 2D results are directly comparable.
