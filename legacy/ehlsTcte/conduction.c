...
main(int argc,char *argv[]) 
{
  ...
  double Tintmax,Tintmin,Tsamax,Tsamin;
  double Tsa1,Tsa0;
  ...
  FILE *f_in;
  double Qcool, Qheat;
  double Tsintmax,Tsintmin,Tsemax,Tsemin,Tintaverage;
  int *juliano;
  ...
  database_begin(user,cont_herramienta,month);
  ...
  while (dias < Days)  {
    ++dias;
    Qcool = Qheat = Tintaverage  = 0.;    Tsintmax = Tsemax = -100.;  Tsintmin = Tsemin = 100.;
    abrefile(a11,a12,a21,e21,e22,e23,A,gamma,&f_in,L4);
    Econd = Econv = Erad = 0.;
    for (t = 0; t <= t_max; t += dt) { 
      ...
      Ein = 0.;
      solve_PQ(a,b,c,d,P,Q,Tn,T,nx,Tint,hi,rhoair,cair,La,&Qin,dt,&Tintaverage,&Qcool,&Qheat);
  ...
  printf("1D  Aa = %f\n\n",Aa); 
  printf("Rr = %f\n\n",RR);
  printf("DF_s\tQcool\tQheat\n");
  printf("%.2f\t",(Tsintmax-Tsintmin)/(Tsemax-Tsemin));                               
  printf("%.2f\t",Qcool/3600.);
  printf("%2\t",Qheat/3600.);
  printf("%f\t%f\t%f\n",Econd/(Econd+Econv+Erad),Econv/(Econd+Econv+Erad),Erad/(Econd+Econv+Erad));

  
  char file_TK[180];
  sprintf(file_TK,"./dat/TK_%d.dat",miniDeltaTK);
