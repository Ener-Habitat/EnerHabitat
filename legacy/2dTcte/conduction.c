/************** conduction.c para aire acondicionado 2D  *************/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
main(int argc,char *argv[]) 
{
  ...
  input_insert("rhocr", "thermal capacity r", &rhocr, 'd');
  ...
  double dt,dx,dy,t,error,t_max,Tsa,Tsurface,Is,Tso,Tsi;
  double **a,**b,**c,**d,**T,**To,**Tn,**Terror,*P,*Q;
  int **NT;
  double **rhoc,**k;
  int i,j,itera;
  double Hi;
  double Qin,rhoair,cair,Tint,Ta,Tc,DtaT,Thueco,hh;
  double Qcool,Qheat;
  double Tsintmax,Tsintmin,Tsemax,Tsemin;
  double Qrup,Qrdown;
  ...
  double Tarriba,Tabajo,Ra,gr,Beta,nu,alphaair,kair,Nur;
  Tarriba = Tabajo = 0.;
  gr = 9.81;
  Beta = 1./300.;
  nu = 1.11e-5;
  ...
  while (error > 1e-5)  { 
    Qin = Tintaverage  = Qrup = Qrdown = Qcool = Qheat = 0.; Tsintmax = Tsemax = -100.;  Tsintmin = Tsemin = 100.;
    Tintmax = Tsamax = -100.;  Tintmin = Tsamin = 100.; animation = 0;
    ...
    for (t = 0; t <= t_max; t += dt) {
      ...
      solve_PQ(a,b,c,d,P,Q,Tn,T,nx,ny,Tint,hi,rhoair,cair,La,&Qin,dt,dx,dy,k,rhoc,NT,Tsa,ho,To,X,t,&Thueco,&hh,i1,j1,i2,j2,a11,a21,a12,e22,E,&Qrup,&Qrdown,&Qcool,&Qheat,tipo,beta,&Tarriba,&Tabajo,&Nur);
      max_min(&Tsemax,&Tsemin,&Tsintmax,&Tsintmin,t,Tint,T,nx,ny,&Tsi);
  ...
  abrefile_indice(user,month,cont_herramienta,&f_in,Tsemax,Tsemin,Tsintmax,Tsintmin,Tint,dt,Qcool,Qheat,X);
